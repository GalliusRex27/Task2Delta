...
var score1=new text1("Score",435,30);
var score=new text1(0,465,30);
...
if(balls[i].x + dt[i].dx > canvas.width-balls[i].r || balls[i].x + dt[i].dx < balls[i].r) {
    ...
    if(balls[i].y + dt[i].dy > canvas.height) {
        vy*=-1;
    }
dt[i].dy+=gravity;
balls[i].x+=dt[i].dx;
...
update();
cancelRequestFrame(drawBullets);
cancelRequestFrame(draw);}}
for(var i=0;i<balls.length;i++){
if(balls[i].x + balls[i].r >  bullet1.x &&
   balls[i].x - balls[i].r < (bullet1.x + bullet1.width) &&
   balls[i].y + balls[i].r >  canvas.height-bullet1.height-r &&
...
score.add1(1);
window.requestAnimFrame(draw,drawBullets,drawPaddle);}
window.requestAnimFrame = function(){
    return (
        function(draw,drawBullets,drawPaddle){
            window.requestAnimationFrame(draw,1000/60);
  window.requestAnimationFrame(drawBullets,1000/60);
  window.requestAnimationFrame(drawPaddle,1000/60);
        }
    );
}();
window.requestAnimFrame(draw,drawBullets,drawPaddle);